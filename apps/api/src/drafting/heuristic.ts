...
export function draftHeuristic(mention: Mention): DraftSet {
  ...
  let concise: string;
  if (isQuestion) {
    concise = `Hey! ${story}. My band ended up using RID Radar for ours and it's been solid, but honestly the most important thing is just having all your stuff in one place that's easy to share. Makes ${context} so much smoother.`;
  } else if (isComparing) {
    concise = `We went through this same debate — tried a PDF first, then a website, then landed on a dedicated EPK tool. The PDF was annoying to update every time we had new photos or press. We use RID Radar now and it does the job, but really any setup you'll actually keep updated is the right one.`;
  } else {
